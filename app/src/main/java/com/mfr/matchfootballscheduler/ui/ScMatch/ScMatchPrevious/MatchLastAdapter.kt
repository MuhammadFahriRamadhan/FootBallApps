...
class MatchLastAdapter(private val Schedulmatch: ArrayList<SchedulMatch>) : RecyclerView.Adapter<MatchLastAdapter.MatchPevViewHolder>() {


    private lateinit var mListenerDetail: (SchedulMatch) -> Unit

    private var convert : String = ""
    fun setOnLastMatchCLickListener(listener: (SchedulMatch) -> Unit) {
        this.mListenerDetail = listener
    }

    override fun onCreateViewHolder(parentitem: ViewGroup, Viewtype: Int): MatchPevViewHolder = MatchPevViewHolder(LayoutInflater.from(parentitem.context).inflate(R.layout.item_match, parentitem, false))

    override fun getItemCount(): Int = Schedulmatch.size

    override fun onBindViewHolder(HolderPrev: MatchPevViewHolder, position: Int) {
        HolderPrev.bindItem(Schedulmatch[position])
    }


    inner class MatchPevViewHolder(view: View) : BaseViewHolder(view) {
    ...
    fun addItemList(schmatches: List<SchedulMatch>) {
        Schedulmatch.clear()
